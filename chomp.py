...
class Chomp(object):
    ...
    def read_stl_triangles(self, file_path):
        self.vertices, indices = stl_reader.read(file_path)
    ...
    def chomp(self):
        ...
        while z <= self.adjusted_max_z:
            layer = self._compute_layer_contours(self.vertices, z)
    ...
    def _compute_layer_contours(self, triangles, z):
        ...
        for triangle in triangles:
            ...
            for (p1, p2, z_p1, z_p2) in [(v0, v1, z0, z1), (v1, v2, z1, z2), (v2, v0, z2, z0)]:
    ...
    def _adjust_z_bounds(self):
        ...
        if (self.orig_min_z < 0):
            self.calculated_layer_count = int((offset+self.orig_max_z)/self.LAYER_HEIGHT)
        else:
            self.calculated_layer_count = int((self.orig_max_z-offset)/self.LAYER_HEIGHT)
